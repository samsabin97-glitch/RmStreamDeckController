...
PORT = r"\\.\COM3"
secondary_PORT = r"\\.\COM4"
BAUD = 9600

# --- single-instance guard ---
LOCK_PATH = r"C:\StreamDeck\Arduino\serial_bridge.lock"
lock = "serial_bridge.lock"

if os.path.exists(lock):
    print("lock file exists -- removing lock inorder to start program")
    try:
        os.remove(lock)
    except Exception as e:
        print("---! couldnt remove lock file error: {e}--")
...
try:
    ...
    print(f"[FAIL] Could not open {PORT}: {e} trying {secondary_PORT}")
    try:
        ser = serial.Serial(secondary_PORT, BAUD, timeout=1, write_timeout=1)
        time.sleep(2.0)  # let Arduino finish boot after port open (DTR reset)
        ser.reset_input_buffer()
        print(f"[OK] Opened {secondary_PORT} @ {BAUD}")
    except Exception as e:
        print(f"[FAIL] Could not open {secondary_PORT}: {e} script or software is likely already using this device")
        sys.exit(1)
